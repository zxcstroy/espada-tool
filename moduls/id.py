...
def check_token_validity(token, proxy):
    ...
    try:
        ...
        if login.status_code == 200:
            print_colored(f"{Fore.GREEN}Вот токен {token}{Fore.RESET}")
            write_to_file(HIT_FILE_PATH, token)
        else:
            print_colored(f"{Fore.RED}{token}{Fore.RESET}")
...
def main():
    ...
    clear_console()
    print_banner()
    print(f"{Fore.MAGENTA}[$]{Style.RESET_ALL}    Dev:{Style.RESET_ALL} {Fore.MAGENTA}vodkadev.ru{Fore.WHITE} <3")
    time.sleep(3)
    clear_console()
